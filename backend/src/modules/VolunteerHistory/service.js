...
//   async addHistoryRecord(userId, eventId, participationStatus, hours, feedback) {
//     const [eventRows] = await db.query(
//       `SELECT title, location, date FROM events WHERE id = ?`,
//       [eventId]
//     );

//     const evt = eventRows[0];

//     const volunteerName = `Volunteer ${userId}`;
...
//     const [rows] = await db.query(`SELECT * FROM volunteer_history WHERE id = ?`, [
//       result.insertId
//     ]);

//     return {
//       ...rows[0],
//       skillsUsed: JSON.parse(rows[0].skillsUsed)
//     };
//   }

//   async getVolunteerStats(userId) {
//     const history = await this.getVolunteerHistory(userId);

//     const totalEvents = history.length;
//     const completed = history.filter(h => h.participationStatus === "completed").length;
//     const totalHours = history.reduce((sum, h) => sum + (h.hoursVolunteered || 0), 0);

//     const allSkills = [
//       ...new Set(history.flatMap(h => (Array.isArray(h.skillsUsed) ? h.skillsUsed : [])))
//     ];
...
//       totalEvents,
//       completed,
//       assigned: totalEvents - completed,
//       totalHours,
